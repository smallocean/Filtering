...
const int AnisoRBFfilterSurfaceFunction::m_numOfIntegralPoint=40;
const double AnisoRBFfilterSurfaceFunction::m_ordinateAndWeightCoff[120]={ 
	//////////////////////////////////////////////////////////////////////////
	/*  
	*   第一列为半径为1时候的x坐标
	*   第二列为半径为1时候的y坐标
...
const double AnisoRBFfilterSurfaceFunction::m_coffRBFunction[5]={8,40,48,25,5};

AnisoRBFfilterSurfaceFunction::AnisoRBFfilterSurfaceFunction(void)
{
	// 初始化所有数据
...
void AnisoRBFfilterSurfaceFunction::GetMeshlessFilter(int numOfPointSet,float* pointSet,float* colors,int kNearest, double radius, int interactiveTime,double namda,	double loadConstant,double ebusainu, int stopN,	int maxIter,double curveThreshold,double functionGradientThreshold)
{
	/*
	函数功能：  接口函数，计算点集滤波
	变量说明：
	...
	double colorThreshold                    颜色阈值
	double variationFunction                 颜色高斯加权方差
	*/
	...
	m_itol=stopN;
	m_itmax=maxIter;
	m_curveThreshold=curveThreshold;
	m_tol=ebusainu;
	m_functionGradientThreshold=functionGradientThreshold;
	...
	m_originalColors=new double[m_numOfPoints*3];
	...
	for(int i=0;i<m_interactiveTime;i++){
		...
		if(m_functionStiffMatrix!=NULL){
			delete[] m_functionStiffMatrix;
			m_functionStiffMatrix=NULL;
		}
		m_functionStiffMatrix=new StiffMatrix[m_numOfPoints*3];
		...
		if(m_loadVectorFunction!=NULL){
			delete[] m_loadVectorFunction;
			m_loadVectorFunction=NULL;
		}
		m_loadVectorFunction=new double[m_numOfPoints*3];
		startTime=clock();
		if(m_functionGradient!=NULL){
			delete[] m_functionGradient;
			m_functionGradient=NULL;
		}
		m_functionGradient=new double[m_numOfPoints*12];		
		for(int j=0;j<m_numOfPoints;j++){

			ComputeNearestParameter(j);
			ComputeFunctionGradient(j);
		...
		for(int tempI=0;tempI<3;tempI++){
			...
			for(int j=0;j<m_numOfPoints;j++){
				std::vector<double>::iterator iteratorFunctionStiffMatrix=m_functionStiffMatrix[j*3+tempI].m_elementStiffMarix.begin();
				for(;iteratorFunctionStiffMatrix!=m_functionStiffMatrix[j*3+tempI].m_elementStiffMarix.end();iteratorFunctionStiffMatrix++){
                    m_stiffMatrix[j].m_elementStiffMarix.push_back((*iteratorFunctionStiffMatrix));
				}
			}
			AdjustStiffMatrix();
			...
			x=new double[m_numOfPoints];
			for(int k=0;k<m_numOfPoints;k++){
				b[k]=m_loadVectorFunction[k*3+tempI];
				x[k]=0;
			}
			int iter;
			double err;
		...
		//写文件for matlab
		//CString filename_pw = "C:\\Program Files\\MATLAB71\\work\\matlabMatrix.txt";
		//FILE *fpout;
		...
		//	fclose(fpout);
		//}
		//filename_pw = "C:\\Program Files\\MATLAB71\\work\\matlabLoadVector.txt";		
		//if((fpout = fopen(filename_pw, "w")) == NULL)
		//{
		//	int dkjkd;
		//	//MessageBox("can't open the file!");
		...
		*/
		...
		for(int j=0;j<m_numOfPoints*3;j++){
			m_functionStiffMatrix[j].m_elementStiffMarix.clear();
		}
...
void AnisoRBFfilterSurfaceFunction::DeleteMeshlessFilter()
{
	/*
	*	函数功能： 释放变量内存
	*/
...
void AnisoRBFfilterSurfaceFunction::ComputeMapKnearest()
{
	/*
	*	函数功能：计算每一个点的 k邻域
	*/
...
void AnisoRBFfilterSurfaceFunction::ComputeNormal()
{
	/*
	*	函数功能：计算每一个点的法向
	*/
...
void AnisoRBFfilterSurfaceFunction::ComputeNearestParameter(int indexPointSet)
{
	/*
...
void AnisoRBFfilterSurfaceFunction::ComputeTestFunction(int indexPointSet)
{
	/*
	*	计算测试函数在积分点的函数值及其导数值
	*/
...
void AnisoRBFfilterSurfaceFunction::ComputeMatrixNeighbor(int indexPointSet)
{
	/*
	*	函数功能：计算插值矩阵及其逆阵
	*  变量说明：
...
void AnisoRBFfilterSurfaceFunction::ComputeMatrixRelatedSampling(int indexPointSet)
{
	/*
	*	函数功能： 计算与采样点有关的矩阵 
	*  变量说明： 
...
void AnisoRBFfilterSurfaceFunction::ComputeShapeFunction(int indexPointSet)
{
	/*
	*	函数功能：计算形函数及其导数
	*  变量说明：
...
void AnisoRBFfilterSurfaceFunction::ComputeManifoldMetric(int indexPointSet)
{
	/*
	*	函数功能： 计算流形标准的代数行列式
	*  变量说明：
...
void AnisoRBFfilterSurfaceFunction::ComputeStiffMatrix(int indexPointSet)
{
	/*
	*	函数功能： 计算刚度矩阵和荷载向量
	*  变量说明： 
...
void AnisoRBFfilterSurfaceFunction::EstimateNormalDirection()
{
	/*
	*	函数功能： 估计法向的准确方向
	*/
...
void AnisoRBFfilterSurfaceFunction::CalculateLinearSystem()
{
	/*
	*	函数功能： 解线性方程
	*/
...
void  AnisoRBFfilterSurfaceFunction::ComputeGradientShapeFunction()
{
	/*
...
void AnisoRBFfilterSurfaceFunction::CalculateLinearSystemGauss()
{
	/*
	*	函数功能： 高斯消元法解方程；
	*/
...
void AnisoRBFfilterSurfaceFunction:: AssembleStiffMatrix()
{
	//////////////////////////////////////////////////////////////////////////	
	/*
	*	函数功能： 组装刚度矩阵
	...
	*   计算函数滤波的刚度矩阵和方程组右边的矩阵，
	*/
	for(int i=0;i<m_numOfPoints;i++){
		double localAnisoConstant;//diffusion coffecient;在这里要加上颜色的影响
		//////////////////////////////////////////////////////////////////////////
		// 计算几何滤波的diffusion系数
		if(m_meanCurvature[i]<0.5*m_curveThreshold){
			localAnisoConstant=1;
		}
		else{
			localAnisoConstant=1/(1+(m_meanCurvature[i]-0.5*m_curveThreshold)*(m_meanCurvature[i]-0.5*m_curveThreshold)/0.25/m_curveThreshold/m_curveThreshold);
		}
		//////////////////////////////////////////////////////////////////////////
		//计算函数滤波的diffusion系数
		double functionWeightCoffecient[3];//函数影响系数
		for(int j=0;j<3;j++){
			if(m_functionGradient[i*12+j*4]<0.5*m_functionGradientThreshold){
				functionWeightCoffecient[j]=1;
			}
			else{
				functionWeightCoffecient[j]=
					1/(1+(m_functionGradient[i*12+j*4]-0.5*m_functionGradientThreshold)*(m_functionGradient[i*12+j*4]-0.5*m_functionGradientThreshold)
					/0.25/m_functionGradientThreshold/m_functionGradientThreshold);
			}
		}
		
		m_loadVector[i]=0;
		m_loadVectorFunction[i*3]=0;
		m_loadVectorFunction[i*3+1]=0;
		m_loadVectorFunction[i*3+2]=0;
		std::vector<double>::iterator iteratorStiffMatrix=m_stiffMatrix[i].m_elementStiffMarix.begin();
		std::vector<double>::iterator iteratorMassMatrix=m_massMatrix[i].m_elementStiffMarix.begin();
		std::map<int, KnearestField>::iterator mapIterator=m_mapKnearest.find(i);
		std::vector<int>::iterator vectorIterator=(*mapIterator).second.m_nearest.begin();
		m_functionStiffMatrix[i*3].m_elementStiffMarix.push_back(0);
		m_functionStiffMatrix[i*3+1].m_elementStiffMarix.push_back(0);
		m_functionStiffMatrix[i*3+2].m_elementStiffMarix.push_back(0);
		...
		if((*mapIterator).second.m_nearest.size()<m_kNearest){
			...
			m_loadVector[i]=1;
			m_loadVectorFunction[i*3]=1;
			m_loadVectorFunction[i*3+1]=1;
			m_loadVectorFunction[i*3+2]=1;
		}
		else{
			/*m_loadVectorFunction[i*3]-=(*iteratorStiffMatrix)*m_resultColor[i*3];
			m_loadVectorFunction[i*3+1]-=(*iteratorStiffMatrix)*m_resultColor[i*3+1];
			m_loadVectorFunction[i*3+2]-=(*iteratorStiffMatrix)*m_resultColor[i*3+2];*/

			for(iteratorStiffMatrix++,iteratorMassMatrix++;iteratorStiffMatrix!=m_stiffMatrix[i].m_elementStiffMarix.end();iteratorStiffMatrix++,vectorIterator++,iteratorMassMatrix++){
				double distanceNormal; //两个法向的点乘
				double distanceProjection;  //领域点到切平面的距离
				double distanceTangent;//邻域点的切向距离
				double distanceColor[3];//颜色距离
				
				double functionGradient[3];//函数梯度
				double elementFunctionStiffmatrix;
				//distanceTangent=(m_resultPointSet[(*vectorIterator)*3]-m_resultPointSet[i*3])*(m_resultPointSet[(*vectorIterator)*3]-m_resultPointSet[i*3])
				//			+(m_resultPointSet[(*vectorIterator)*3+1]-m_resultPointSet[i*3+1])*(m_resultPointSet[(*vectorIterator)*3+1]-m_resultPointSet[i*3+1])
				//			+(m_resultPointSet[(*vectorIterator)*3+2]-m_resultPointSet[i*3+2])*(m_resultPointSet[(*vectorIterator)*3+2]-m_resultPointSet[i*3+2]);
				...
				//distanceTangent=sqrt(distanceTangent-distanceProjection*distanceProjection);
				distanceColor[0]=m_resultColor[(*vectorIterator)*3]-m_resultColor[i*3];
				distanceColor[1]=m_resultColor[(*vectorIterator)*3+1]-m_resultColor[i*3+1];
				distanceColor[2]=m_resultColor[(*vectorIterator)*3+2]-m_resultColor[i*3+2];
				/*if(distanceTangent<0.0000001){
					functionGradient[0]=0;
					functionGradient[1]=0;
					functionGradient[2]=0;					
				}
				else{
					functionGradient[0]=abs(distanceColor[0]/distanceTangent);
					functionGradient[1]=abs(distanceColor[1]/distanceTangent);
					functionGradient[2]=abs(distanceColor[2]/distanceTangent);
				}*/
				/*for(int tempI=0;tempI<3;tempI++){
					if(functionGradient[tempI]<0.5*m_functionGradientThreshold){
						functionWeightCoffecient[tempI]=1;
					}
					else{
						functionWeightCoffecient[tempI]=1/(1+(functionGradient[tempI]-0.5*m_functionGradientThreshold)
										*(functionGradient[tempI]-0.5*m_functionGradientThreshold)/0.25/m_functionGradientThreshold/m_functionGradientThreshold);
					}
				}	*/
				...
				m_loadVectorFunction[i*3+0]-=(*iteratorStiffMatrix)*(m_resultColor[(*vectorIterator)*3]-m_resultColor[i*3])*functionWeightCoffecient[0];
				m_loadVectorFunction[i*3+1]-=(*iteratorStiffMatrix)*(m_resultColor[(*vectorIterator)*3+1]-m_resultColor[i*3+1])*functionWeightCoffecient[1];
				m_loadVectorFunction[i*3+2]-=(*iteratorStiffMatrix)*(m_resultColor[(*vectorIterator)*3+2]-m_resultColor[i*3+2])*functionWeightCoffecient[2];
				(*iteratorStiffMatrix)=((*iteratorStiffMatrix))*localAnisoConstant;
				for(int tempI=0;tempI<3;tempI++){
					elementFunctionStiffmatrix=((*iteratorStiffMatrix))*localAnisoConstant*functionWeightCoffecient[tempI]; 
					m_functionStiffMatrix[i*3+tempI].m_elementStiffMarix.push_back(elementFunctionStiffmatrix);
				}			
				/*m_loadVector[i]-=(*iteratorStiffMatrix)*distanceProjection;

				(*iteratorStiffMatrix)=((*iteratorStiffMatrix))*localAnisoConstant/distanceNormal;	*/
				//	(*iteratorStiffMatrix)=((*iteratorStiffMatrix)+(*iteratorMassMatrix))*distanceNormal;		

			}
			m_loadVector[i]*=localAnisoConstant;
			m_loadVectorFunction[i*3]*=localAnisoConstant;
			m_loadVectorFunction[i*3+1]*=localAnisoConstant;
			m_loadVectorFunction[i*3+2]*=localAnisoConstant;
		}
	}
	//////////////////////////////////////////////////////////////////////////
	/*
	...
	*/
	...
	for (int i=0;i<m_numOfPoints;i++){
		std::vector<double>::iterator iteratorStiffMatrix=m_stiffMatrix[i].m_elementStiffMarix.begin();
		std::vector<double>::iterator iteratorFunctionStiffMatrixOne=m_functionStiffMatrix[i*3].m_elementStiffMarix.begin();
		std::vector<double>::iterator iteratorFunctionStiffMatrixTwo=m_functionStiffMatrix[i*3+1].m_elementStiffMarix.begin();
		std::vector<double>::iterator iteratorFunctionStiffMatrixThree=m_functionStiffMatrix[i*3+2].m_elementStiffMarix.begin();
		...
		//	iteratorMassMatrix++;
		//vectorIterator++;
		iteratorFunctionStiffMatrixOne++;
		iteratorFunctionStiffMatrixTwo++;
		iteratorFunctionStiffMatrixThree++;
		for(iteratorStiffMatrix++;iteratorStiffMatrix!=m_stiffMatrix[i].m_elementStiffMarix.end();iteratorStiffMatrix++,vectorIterator++){
			...
			std::vector<double>::iterator iteratorFunctionStiffMatrixOneIner=m_functionStiffMatrix[(*vectorIterator)*3].m_elementStiffMarix.begin();
			std::vector<double>::iterator iteratorFunctionStiffMatrixTwoIner=m_functionStiffMatrix[(*vectorIterator)*3+1].m_elementStiffMarix.begin();
			std::vector<double>::iterator iteratorFunctionStiffMatrixThreeIner=m_functionStiffMatrix[(*vectorIterator)*3+2].m_elementStiffMarix.begin();
			
			bool temp=false;	
			//vectorIteratorIner++;
			iteratorFunctionStiffMatrixOneIner++;
			iteratorFunctionStiffMatrixTwoIner++;
			iteratorFunctionStiffMatrixThreeIner++;
			...
			for(iteratorStiffMatrixIner++;iteratorStiffMatrixIner!=m_stiffMatrix[(*vectorIterator)].m_elementStiffMarix.end();iteratorStiffMatrixIner++,vectorIteratorIner++){
				if((*vectorIteratorIner)==i){
					...
					(*iteratorFunctionStiffMatrixOne)=((*iteratorFunctionStiffMatrixOne)+(*iteratorFunctionStiffMatrixOneIner))/2;
					(*iteratorFunctionStiffMatrixOneIner)=(*iteratorFunctionStiffMatrixOne);
					(*iteratorFunctionStiffMatrixTwo)=((*iteratorFunctionStiffMatrixTwo)+(*iteratorFunctionStiffMatrixTwoIner))/2;
					(*iteratorFunctionStiffMatrixTwoIner)=(*iteratorFunctionStiffMatrixTwo);
					(*iteratorFunctionStiffMatrixThree)=((*iteratorFunctionStiffMatrixThree)+(*iteratorFunctionStiffMatrixThreeIner))/2;
					(*iteratorFunctionStiffMatrixThreeIner)=(*iteratorFunctionStiffMatrixThree);
			...
			if(temp==false){
				...
				(*iteratorFunctionStiffMatrixOne)/=2;
				m_functionStiffMatrix[(*vectorIterator)*3].m_elementStiffMarix.push_back((*iteratorFunctionStiffMatrixOne));
				(*iteratorFunctionStiffMatrixTwo)/=2;
				m_functionStiffMatrix[(*vectorIterator)*3+1].m_elementStiffMarix.push_back((*iteratorFunctionStiffMatrixTwo));
				(*iteratorFunctionStiffMatrixThree)/=2;
				m_functionStiffMatrix[(*vectorIterator)*3+2].m_elementStiffMarix.push_back((*iteratorFunctionStiffMatrixThree));
	...
	for(int i=0;i<m_numOfPoints;i++){
		...
		double sumOfElement=0;
		double sumOfFunctionElement[3];
		for(int tempI=0;tempI<3;tempI++){
			sumOfFunctionElement[tempI]=0;
		}
		iteratorMassMatrix++;
		iteratorFunctionStiffMatrixOne++;
		iteratorFunctionStiffMatrixTwo++;
		...
		for(iteratorStiffMatrix++;iteratorStiffMatrix!=m_stiffMatrix[i].m_elementStiffMarix.end();iteratorStiffMatrix++,iteratorMassMatrix++){
			sumOfElement+=(*iteratorStiffMatrix);
			sumOfFunctionElement[0]+=(*iteratorFunctionStiffMatrixOne);
			sumOfFunctionElement[1]+=(*iteratorFunctionStiffMatrixTwo);
			sumOfFunctionElement[2]+=(*iteratorFunctionStiffMatrixThree);
		...
		iteratorStiffMatrix=m_stiffMatrix[i].m_elementStiffMarix.begin();
		iteratorMassMatrix=m_massMatrix[i].m_elementStiffMarix.begin();
		iteratorFunctionStiffMatrixOne=m_functionStiffMatrix[i*3].m_elementStiffMarix.begin();
		iteratorFunctionStiffMatrixTwo=m_functionStiffMatrix[i*3+1].m_elementStiffMarix.begin();
		iteratorFunctionStiffMatrixThree=m_functionStiffMatrix[i*3+2].m_elementStiffMarix.begin();
		/*for(int j=0;j<3;j++){
		...
		(*iteratorStiffMatrix)=(*iteratorMassMatrix)-sumOfElement;
		(*iteratorFunctionStiffMatrixOne)=(*iteratorMassMatrix)-sumOfFunctionElement[0];
		(*iteratorFunctionStiffMatrixTwo)=(*iteratorMassMatrix)-sumOfFunctionElement[1];
		(*iteratorFunctionStiffMatrixThree)=(*iteratorMassMatrix)-sumOfFunctionElement[2];
	}
	return;
}
//////////////////////////////////////////////////////////////////////////
...
*	调整刚度矩阵
*/
void AnisoRBFfilterSurfaceFunction::AdjustStiffMatrix()
{
	for (int i=0;i<m_numOfPoints;i++){
...
void AnisoRBFfilterSurfaceFunction::linbcg(double* b,double* x,int itol,double tol,int itmax, int & iter, double &err)
{
...
double AnisoRBFfilterSurfaceFunction::snrm(double* sx,const int itol)
{
	int i,isamax;
	double ans;
	int n=m_numOfPoints;
...
void AnisoRBFfilterSurfaceFunction::atimes(double* x,double* r,const int itrnsp)
{
	if(itrnsp==0){
		for (int i=0;i<m_numOfPoints;i++){
			std::vector<double>::iterator iteratorStiffMatrix=m_stiffMatrix[i].m_elementStiffMarix.begin();
...
void AnisoRBFfilterSurfaceFunction::asolve(double* b,double* x,const int itrnsp)
{
	int i;
	int n=m_numOfPoints;
	for(i=0;i<n;i++){
...
void AnisoRBFfilterSurfaceFunction::ComputeFunctionGradient(int indexPointSet)
{
	//////////////////////////////////////////////////////////////////////////
	/*
	 *	共分四个步骤
	 ...
	 *  2. 对方向导数做矢量分解
	 *  3. 对分解后的矢量做PCA分析
	 *  4. 求的最大特制值为梯度大小，方向为梯度方向
	 */
	int tempNumOfNeighbors=m_numOfNeighbors-1;
	double* tempDirectionDerivativeVector;
	tempDirectionDerivativeVector=new double[tempNumOfNeighbors*6];
	std::map<int, KnearestField>::iterator mapIterator=m_mapKnearest.find(indexPointSet);
	std::vector<int>::iterator vectorIterator=(*mapIterator).second.m_nearest.begin();

	//////////////////////////////////////////////////////////////////////////
	// 求方向导数并对做矢量分解
	for(int i=1;vectorIterator!=(*mapIterator).second.m_nearest.end();vectorIterator++,i++){//i表示领域点的顺序
		double tempColorDifference[3];
		double tempDistance;
		tempDistance=sqrt(m_localOrdinate[i*2]*m_localOrdinate[i*2]+m_localOrdinate[i*2+1]*m_localOrdinate[i*2+1]);
		if(tempDistance<0.000001){
			for(int j=0;j<3;j++){
				tempDirectionDerivativeVector[(i-1)*6+j*2]=0;
				tempDirectionDerivativeVector[(i-1)*2+j*2+1]=0;
			}
		}
		else{
			for(int j=0;j<3;j++){
				double tempDirectionDerivative;
				tempColorDifference[j]=m_resultColor[indexPointSet*3+j]-m_resultColor[(*vectorIterator)*3+j];
				tempDirectionDerivative=tempColorDifference[j]/tempDistance;
				tempDirectionDerivativeVector[(i-1)*6+j*2]=abs(tempDirectionDerivative*m_localOrdinate[i*2]/tempDistance);
				tempDirectionDerivativeVector[(i-1)*2+j*2+1]=abs(tempDirectionDerivative*m_localOrdinate[i*2+1]/tempDistance);
			}       
	...
	//////////////////////////////////////////////////////////////////////////
	// 做PCA分析
	for(int i=0;i<3;i++){// i表示三个函数，及三颜色
		mat_f8 mat_covaMatrix(2, 2);
		//求方差矩阵
		for(int j=0;j<2;j++){
			for(int k=0;k<2;k++){
				mat_covaMatrix(j,k)=0;
				for(int m=0;m<tempNumOfNeighbors;m++){
					mat_covaMatrix(j,k)=mat_covaMatrix(j,k)+tempDirectionDerivativeVector[m*6+i*2+j]*tempDirectionDerivativeVector[m*6+i*2+k];
				}
			}
		}
		vec_f8 eval(2);
		mat_f8 evec(2, 2);
		eigen_symm(eval, evec, mat_covaMatrix);
		m_functionGradient[indexPointSet*12+i*4]=eval(0);
		m_functionGradient[indexPointSet*12+i*4+1]=eval(1);
		m_functionGradient[indexPointSet*12+i*4+2]=evec(0,0);
		m_functionGradient[indexPointSet*12+i*4+3]=evec(1,0);
		if(m_functionGradient[indexPointSet*12+i*4]<0||m_functionGradient[indexPointSet*12+i*4+1]<0){
			int www;
			www=0;
		}
		if(m_functionGradient[indexPointSet*12+i*4]<m_functionGradient[indexPointSet*12+i*4+1]<0){
			int www;
			www=0;
		}
	}
    delete[] tempDirectionDerivativeVector;
}