...
const int MLSfilter::m_numOfInterpolatePoint=1;
const int MLSfilter::m_rankOfInterpolate=3;

const float MLSfilter::m_localInterpolatePoint[9][2]={
...
const float MLSfilter::m_TestFunction[9]={
	0.00646952576012,
		0.09626480766995,
		0.00646952576012,
		0.09626480766995,
...
const float MLSfilter::m_TestFunctionDerivativeOne[9]={
	0.04510145794586,
		0,
		-0.04510145794586,
		0.67109759444089,
...
const float MLSfilter::m_TestFunctionDerivativeTwo[9]={
	0.04510145794586, 
		0.67109759444089,
		0.04510145794586,
		0,
...
const float MLSfilter::m_gaussItegral[9]={
...
MLSfilter::MLSfilter(void)
{
	m_originalPointSet=NULL;
	 m_localDistance=NULL;
...
void  MLSfilter::GetMLSfilter(int numOfPointSet,float* pointSet,int kNearest, float radius,int interactiveTime)
{
	/*
	函数功能：  接口函数，计算点集滤波
	变量说明：
	...
	*/
	...
	for(int i=0;i<m_interactiveTime;i++){
		...
		for(int j=0;j<m_numOfPoints;j++){
			...
			double localMoveDistance=0;
			for(int k=0;k<m_numOfNeighbors;k++){
				localMoveDistance+=m_localOrdinate[k*3+2]*m_shapeFunction[k+1];				
			}
			for(int www=0;www<3;www++){
				m_resultPointSet[j*3+www]=m_originalPointSet[j*3+www]+localMoveDistance*m_originalNormals[j*3+www];
			}
		...
		for(int j=0;j<m_numOfPoints;j++){
			for(int k=0;k<3;k++){
				m_originalPointSet[j*3+k]=m_resultPointSet[j*3+k];
			}
		}
		//////////////////////////////////////////////////////////////////////////
		// 解线性系统
...
void MLSfilter:: DeleteMLSfilter()
{
	if(m_originalPointSet!=NULL)
		delete[] m_originalPointSet;
	m_originalPointSet=NULL;
...
void MLSfilter::ComputeMapKnearest()
{
	...
	//////////////////////////////////////////////////////////////////////////
	//// 测试代码
	//CString filename_pw = "D:\\qin\\MLSfilter\\CeshiData\\knearest.txt";
	//FILE *fpout;
...
void MLSfilter::ComputeNormal()
{
	...
	for(int i=0;i<m_numOfPoints;i++){
		...
		if(m_meanCurvature[i]*4<=0.0005){
			m_resultColor[i*3]=0;
			m_resultColor[i*3+1]=0;
			m_resultColor[i*3+2]=0.5+80*m_meanCurvature[i]*4;
		}
		if((m_meanCurvature[i]*4>0.0005)&(m_meanCurvature[i]*4<=0.001)){
			m_resultColor[i*3]=0;
			m_resultColor[i*3+1]=0.45+50*(m_meanCurvature[i]*4-0.0005);
			m_resultColor[i*3+2]=0;
		}
		if(m_meanCurvature[i]*4>0.001){
			m_resultColor[i*3]=0.35+5*(m_meanCurvature[i]*4-0.001);
			m_resultColor[i*3+1]=0;
			m_resultColor[i*3+2]=0;
	...
	//////////////////////////////////////////////////////////////////////////
	// 写法向文件
	CString filename_pw = "D:\\hxy\\Bunny\\normal.txt";
	FILE *fpout;
	if((fpout = fopen(filename_pw, "w")) == NULL)
	{
		int dkjkd;
	...
	////////////////////////////////////////////////////////////////////////////
	//// 写主方向文件
	//filename_pw = "D:\\hxy\\Bunny\\majorDirection.txt";
	...
	////////////////////////////////////////////////////////////////////////////
	//// 写次方向文件
	//filename_pw = "D:\\hxy\\Bunny\\minorDirection.txt";
...
void MLSfilter::ComputeNearestParameter(int indexPointSet)
{
	//int numOfNearest;
	...
	if(m_localOrdinate!=NULL)
		delete[] m_localOrdinate;
	m_localOrdinate=NULL;
	m_localOrdinate=new float[m_numOfNeighbors*3];
	if(m_localOrdinate!=NULL){
		delete[] m_localDistance;
		m_localDistance=NULL;
	}
	m_localDistance=new double[m_numOfNeighbors];
	...
	for(int i=0;vectorIterator!=(*mapIterator).second.m_nearest.end();vectorIterator++,i++){
		...
		m_localDistance[i]=0;
		for(int j=0;j<3;j++){
			vectorNeighborToPoint[j]=m_originalPointSet[(*vectorIterator)*3+j]-m_originalPointSet[indexPointSet*3+j];
			m_localDistance[i]+=vectorNeighborToPoint[j]*vectorNeighborToPoint[j];
		}
    ...
    m_localWeightRadiusSquare=m_localDistance[m_numOfNeighbors-1];
...
*/
	...
	//////////////////////////////////////////////////////////////////////////
	// 写局部坐标文件
	//CString filename_pw = "D:\\qin\\MLSfilter\\CeshiData\\localOrdinalte.txt";
	//FILE *fpout;
...
void MLSfilter::ComputeQmatrix(int indexPointSet)
{
	...
	//////////////////////////////////////////////////////////////////////////
	// 写文件
	//CString filename_pw = "D:\\qin\\MLSfilter\\CeshiData\\localQmatrix.txt";
	//FILE *fpout;
	//if((fpout = fopen(filename_pw, "a")) == NULL)
	//{
	//	int dkjkd;
...
void MLSfilter::ComputeMatrixRelatedSampling(int indexPointSet)
{
	/*
	*	函数功能： 计算与采样点有关的矩阵
	*  变量说明： 
	...
	*/
	...
	/*	float* QTweightQDerivativeOne=new float[m_rankOfInterpolate*m_rankOfInterpolate];
		...
		localInterpolatePoint[0]=0;//m_localInterpolatePoint[i][0]*m_localIntergralRadius;
		localInterpolatePoint[1]=0;//m_localInterpolatePoint[i][1]*m_localIntergralRadius;

		weightVector[0]=exp(0.0);
	...
	/*		float localOrdinateMinus[2];
			...
			weightVector[j+1]=exp(-0.5*m_localDistance[j]/m_localWeightRadiusSquare);
	...
	//CString filename_pw = "D:\\qin\\MLSfilter\\CeshiData\\kernelShapeFunction.txt";
	//FILE *fpout;
	//if((fpout = fopen(filename_pw, "a")) == NULL)
	//{
	//	int dkjkd;
	...
	//filename_pw = "D:\\qin\\MLSfilter\\CeshiData\\kernelDerivativeOneShapeFunctionOne.txt";
	...
	//filename_pw = "D:\\qin\\MLSfilter\\CeshiData\\kernelDerivativeTwoShapeFunctionOne.txt";
	...
	//filename_pw = "D:\\qin\\MLSfilter\\CeshiData\\kernelDerivativeOneShapeFunctionTwo.txt";
	...
	//filename_pw = "D:\\qin\\MLSfilter\\CeshiData\\kernelDerivativeTwoShapeFunctionTwo.txt";
...
void MLSfilter::ComputeShapeFunction(int indexPointSet)
{
	/*
	*	函数功能：计算形函数及其导数
	*  变量说明：
	...
	*/
	...
	for(int i=0;i<m_numOfInterpolatePoint;i++){
		...
		localInterFunction[i*m_rankOfInterpolate+1]=0;
		localInterFunction[i*m_rankOfInterpolate+2]=0;
	...
	//////////////////////////////////////////////////////////////////////////
	/// 写文件 shapefunction
	//CString filename_pw = "D:\\qin\\MLSfilter\\CeshiData\\shapeFunction.txt";
	//FILE *fpout;
	//if((fpout = fopen(filename_pw, "a")) == NULL)
	//{
	//	int dkjkd;
	...
	//filename_pw = "D:\\qin\\MLSfilter\\CeshiData\\shapeFunctionDerivativeOne.txt";
	...
	//filename_pw = "D:\\qin\\MLSfilter\\CeshiData\\shapeFunctionDerivativeTwo.txt";
...
void MLSfilter::ComputeManifoldMetric(int indexPointSet)
{
	/*
	*	函数功能： 计算流形标准的代数行列式
	*  变量说明：
	...
	*/
	...
	//////////////////////////////////////////////////////////////////////////
	///// 写文件 shapefunction
	//CString filename_pw = "D:\\qin\\MLSfilter\\CeshiData\\deteMetric.txt";
	//FILE *fpout;
	//if((fpout = fopen(filename_pw, "a")) == NULL)
	//{
	//	int dkjkd;
...
void MLSfilter::ComputeStiffMatrix(int indexPointSet)
{
	//////////////////////////////////////////////////////////////////////////
	//首先计算 对角线上的元素
	...
	//////////////////////////////////////////////////////////////////////////
	/// 写文件 刚度矩阵
	//CString filename_pw = "D:\\qin\\MLSfilter\\CeshiData\\stiffMatrix.txt";
	//FILE *fpout;
	//if((fpout = fopen(filename_pw, "a")) == NULL)
	//{
	//	int dkjkd;
...
void MLSfilter::EstimateNormalDirection()
{
	//首先确定z值最大的那一点的法向
	//首先确定z值最大的那一点的法向
	int maxZpoint=0;
...
void MLSfilter::CalculateLinearSystem()
{
