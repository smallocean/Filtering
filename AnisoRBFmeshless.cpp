	...
	const int AnisoRBFmeshless::m_numOfIntegralPoint=40;
	const double AnisoRBFmeshless::m_ordinateAndWeightCoff[120]={ 
	//////////////////////////////////////////////////////////////////////////
	/*  
	*   第一列为半径为1时候的x坐标
	*   第二列为半径为1时候的y坐标
	...
	*/
	...
	const double AnisoRBFmeshless::m_coffRBFunction[5]={8,40,48,25,5};

	AnisoRBFmeshless::AnisoRBFmeshless(void)
	{
	// 初始化所有数据
	...
	void AnisoRBFmeshless::GetMeshlessFilter(int numOfPointSet,float* pointSet,int kNearest, double radius, int interactiveTime,double namda,	double loadConstant,double ebusainu, int stopN,	int maxIter,double curveThreshold)
	{
	/*
	函数功能：  接口函数，计算点集滤波
	变量说明：
	...
	*/
	...
	void AnisoRBFmeshless::DeleteMeshlessFilter()
	{
	/*
	*	函数功能： 释放变量内存
	*/
	...
	void AnisoRBFmeshless::ComputeMapKnearest()
	{
	/*
	*	函数功能：计算每一个点的 k邻域
	*/
	...
	void AnisoRBFmeshless::ComputeNormal()
	{
	/*
	*	函数功能：计算每一个点的法向
	*/
	...
	void AnisoRBFmeshless::ComputeNearestParameter(int indexPointSet)
	{
	/*
	...
	*/
	...
	void AnisoRBFmeshless::ComputeTestFunction(int indexPointSet)
	{
	/*
	*	计算测试函数在积分点的函数值及其导数值
	*/
	...
	*/
	...
	void AnisoRBFmeshless::ComputeMatrixNeighbor(int indexPointSet)
	{
	/*
	*	函数功能：计算插值矩阵及其逆阵
	*  变量说明：
	...
	*/
	...
	for(int i=0;i<m_numOfNeighbors;i++){
		...
		for(int j=0;j<m_numOfNeighbors;j++){
			...
			if(tempDistance>m_localRBFradius){
				...
				// tempOne is the first form of RBF
	...
	void AnisoRBFmeshless::ComputeMatrixRelatedSampling(int indexPointSet)
	{
	/*
	*	函数功能： 计算与采样点有关的矩阵 
	*  变量说明： 
	...
	*/
	...
	void AnisoRBFmeshless::ComputeShapeFunction(int indexPointSet)
	{
	/*
	*	函数功能：计算形函数及其导数
	*  变量说明：
	...
	*/
	...
	void AnisoRBFmeshless::ComputeManifoldMetric(int indexPointSet)
	{
	/*
	*	函数功能： 计算流形标准的代数行列式
	*  变量说明：
	...
	*/
	...
	void AnisoRBFmeshless::ComputeStiffMatrix(int indexPointSet)
	{
	/*
	*	函数功能： 计算刚度矩阵和荷载向量
	*  变量说明： 
	...
	*/
	...
	for(int i=0;i<m_numOfNeighbors;i++){
		...
		for(int j=0;j<m_numOfIntegralPoint;j++){
			...
			elementTwo+=(m_shapeFunctionDerOne[i*m_numOfIntegralPoint+j]*m_testFunctionDerOne[j]
						 +m_shapeFunctionDerTwo[i*m_numOfIntegralPoint+j]*m_testFunctionDerTwo[j])
						 *m_ordinateAndWeightCoff[j*3+2];
			/*	double tempElementTwo=0;
			for(int k=0;k<3;k++){
	...
	void AnisoRBFmeshless::EstimateNormalDirection()
	{
	/*
	*	函数功能： 估计法向的准确方向
	*/
	...
	void AnisoRBFmeshless::CalculateLinearSystem()
	{
	/*
	*	函数功能： 解线性方程
	*/
	...
	void  AnisoRBFmeshless::ComputeGradientShapeFunction()
	{
	/*
	...
	*/
	...
	void AnisoRBFmeshless::CalculateLinearSystemGauss()
	{
	/*
	*	函数功能： 高斯消元法解方程；
	*/
	...
	void AnisoRBFmeshless:: AssembleStiffMatrix(){
	//////////////////////////////////////////////////////////////////////////	
	/*
	*	函数功能： 组装刚度矩阵
	*/
	...
	for(int i=0;i<m_numOfPoints;i++){
		...
		if((*mapIterator).second.m_nearest.size()<m_kNearest){
			...
			for(iteratorStiffMatrix++;
				iteratorStiffMatrix!=m_stiffMatrix[i].m_elementStiffMarix.end();
				iteratorStiffMatrix++,vectorIterator++){
	...
	*	调整刚度矩阵
	*/
	void AnisoRBFmeshless::AdjustStiffMatrix()
	{
	for (int i=0;i<m_numOfPoints;i++){
	...
	*/
	...
	void AnisoRBFmeshless::linbcg(double* b,double* x,int itol,double tol,int itmax, int & iter, double &err)
	{
	...
	double AnisoRBFmeshless::snrm(double* sx,const int itol)
	{
	int i,isamax;
	double ans;
	int n=m_numOfPoints;
	...
	void AnisoRBFmeshless::atimes(double* x,double* r,const int itrnsp)
	{
	if(itrnsp==0){
		for (int i=0;i<m_numOfPoints;i++){
			std::vector<double>::iterator iteratorStiffMatrix=m_stiffMatrix[i].m_elementStiffMarix.begin();
	...
	void AnisoRBFmeshless::asolve(double* b,double* x,const int itrnsp)
	{
	int i;
	int n=m_numOfPoints;
	for(i=0;i<n;i++){
