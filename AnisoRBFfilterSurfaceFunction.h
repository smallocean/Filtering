...
*   All rights reserved
*
*   filename: AnisoRBFfilterSurfaceFunction
*   abstract: 建立在MLPG方法基础上，选择RBF为试函数的表面和定义在表面上的函数的滤波
*
*   coder: redstar
*   date: 2005/11/12
*
...
class AnisoRBFfilterSurfaceFunction
{
public:
	AnisoRBFfilterSurfaceFunction(void);
	~AnisoRBFfilterSurfaceFunction(void);
public:
...
protected:
	...
	double* m_loadVectorFunction;//函数滤波时候的荷载向量
	double* m_loadVector;//几何滤波时候的荷载向量
	StiffMatrix* m_stiffMatrix;// 刚度矩阵	
	StiffMatrix* m_massMatrix;//质量矩阵
	StiffMatrix* m_functionStiffMatrix;//函数滤波的刚度矩阵
	double m_namda;// 时间步长
	double m_loadConstant;//荷载常数
	double m_ebusainu;//循环停止的条件；
	double* m_meanCurvature;//平均曲率
	double m_curveThreshold;//曲率阈值
	double m_functionGradientThreshold;//函数滤波时候函数梯度的阈值
	double m_variationFunctin;//计算函数加权系数的方差
	double* m_functionGradient;//保存函数的方向导数 每一个点保存四个数，1. 最大的方向导数的大小
	                            // 2. 最小的方向导数的大小 3. 一个二维向量，表示梯度的方向
...
public:
	void GetMeshlessFilter(int numOfPointSet,float* pointSet,float* colors,int kNearest, double radius, int interactiveTime,double namda,	double loadConstant,double ebusainu, int stopN,	int maxIter,double curveThreshold,double functionGradientThreshold);
...
protected:
	...
	*/
	 ...
	 *	计算函数梯度
	 */
	void ComputeFunctionGradient(int indexPointSet);
